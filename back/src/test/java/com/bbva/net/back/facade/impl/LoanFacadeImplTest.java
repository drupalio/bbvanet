...
public class LoanFacadeImplTest {
    ...
    public void init() {
        ...
        Mockito.when(
                productService.select(Matchers.any(GlobalProductsDto.class), Matchers.any(VisibleProductPredicate.class)))
                .thenReturn(globalProduct);
        Mockito.when(
                productService.select(Matchers.any(GlobalProductsDto.class), Matchers.any(HiddenProductPredicate.class)))
                .thenReturn(globalProduct);
        // Set
    ...
    public void getLoansByUser() {
        ...
        Mockito.verify(this.productService, Mockito.atLeastOnce()).select(Matchers.any(GlobalProductsDto.class),
                Matchers.any(VisibleProductPredicate.class));
    ...
    public void getLoansByUserHidden() {
        ...
        // Verificar método select
        Mockito.verify(this.productService, Mockito.atLeastOnce()).select(Matchers.any(GlobalProductsDto.class),
                Matchers.any(HiddenProductPredicate.class));
        Mockito.verify(this.globalProduct, Mockito.atLeastOnce()).getLoan();
    }
