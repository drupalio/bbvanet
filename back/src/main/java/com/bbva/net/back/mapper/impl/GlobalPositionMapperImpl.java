...
public class GlobalPositionMapperImpl extends ConfigurableMapper implements GlobalPositionMapper {
	...
	public GlobalProductsDto map(final List<Product> products) {
		...
		new ProductVisitorCommand(products) {
			...
			public void executeRotatingAccount(final Product rotatingAccount) {
				LOGGER.info("Mapper of globalPosition, execute Rotating Account "+ rotatingAccount.getId());
			...
			public void executeLoan(final Product loan) {
				LOGGER.info("Mapper of globalPosition, execute loan "+ loan.getId());
			...
			public void executeLeasing(final Product leasing) {
				LOGGER.info("Mapper of globalPosition, execute Leasing "+ leasing.getId());
			...
			public void executeFund(final Product fund) {
				LOGGER.info("Mapper of globalPosition, execute Fund "+ fund.getId());
			...
			public void executeDeposit(final Product deposit) {
				LOGGER.info("Mapper of globalPosition, execute Deposit "+ deposit.getId());
			...
			public void executeCredictCard(final Product creditCard) {
				LOGGER.info("Mapper of globalPosition, execute CreditCard "+ creditCard.getId());
			...
			public void executeAdquirenceAccount(final Product adquirenceAccount) {
				LOGGER.info("Mapper of globalPosition, execute Adquirence Account "+ adquirenceAccount.getId());
			...
			public void executeAccount(final Product account) {
				LOGGER.info("Mapper of globalPosition, execute Account "+ account.getId());
	...
	protected void configure(final MapperFactory factory) {
		...
		factory.classMap(Product.class, ProductDto.class).mapNulls(false).mapNullsInReverse(false)
