...
public class QuotaPaginatedControllerTest extends AbstractBbvaControllerTest {
	...
	public void checkNextPage() {
		Mockito.when(quotaDetailFacade.listRotaryQuotaMovements(DEFAULT_ID, daterange, 0, 10)).thenReturn(quotaList);
		List<MovementDto> result = this.quotaPaginatedController.getNextPage(0, 10);
	...
	public void checkgetNextPaginantionKey() {
		Integer result = this.quotaPaginatedController.getNextPaginantionKey(quotaList);
		Assert.assertNotNull(result);
		Whitebox.setInternalState(quotaList, "elementData", new Object[15]);
		Whitebox.setInternalState(quotaList, "size", 15);
		quotaList.set(14, new MovementDto("123456", null, null, null, null, null, null, null, null));
